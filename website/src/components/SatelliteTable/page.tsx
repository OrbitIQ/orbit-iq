...
const sanitizeSatelliteDataJson = (data: SatelliteData): SatelliteData => {
  data.satellites.forEach((satellite) => {
    satellite.source_satellite = sanitizeSourceSatellite(satellite.source_satellite);
...
const sanitizeSourceSatellite = (source_satellite: Array<null | string>): Array<null | string> =>{
...
export default function SatelliteTable() {
  ...
  useEffect(() => {
     const getData = async () =>{
     try{
         const satelliteData = await Axios.get<SatelliteData>(confirmedSatellitesURL);
